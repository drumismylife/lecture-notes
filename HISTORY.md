...
정리·핵심 원어 5개·본문 강해 8곳·신약 인용 4곳·참고도서 6권 소개·묵상질문) 제작, `data.js`의
`prophet.tools[]`에 등록해 사이트 상단에 노출(주차 노트와 별개 채널 — `deploy.py`가 다루는
"주차" 개념과 달라 수동으로 `add_back_button.py` 실행 후 `tools` 배열에 직접 등록하는 절차를
새로 확립). 앞으로 다른 책(요엘·아모스 등, 신약성서 각 권)에도 같은 형식을 반복 적용할 것.
